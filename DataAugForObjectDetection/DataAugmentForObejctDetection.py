...
class DataAugmentForObjectDetection():
	...
	def dataAugment(self, img, bboxes, aug_num):
		'''
		图像增强
		输入:
			img:图像array
		...
		change_num = 0  #改变的次数
		print('------')
		while change_num < aug_num:   #默认至少有一种数据增强生效
...
if __name__ == '__main__':
	...
	for parent, _, files in os.walk(source_pic_root_path):
		for file in files:
			...
			auged_img, auged_bboxes = dataAug.dataAugment(img, coords, need_aug_num)

			tmp_name = '%s/%s.jpg' %(img_root_path, str(start_name))
			cv2.imwrite(tmp_name, auged_img)
			tmp_ano_name = '%s/%s.xml' %(aug_root_path, str(start_name))
			generate_xml(tmp_name, auged_bboxes, auged_img.shape, tmp_ano_name)
			start_name += 1
